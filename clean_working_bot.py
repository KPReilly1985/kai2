...
def get_config(config_path=None):
    return SimpleNamespace(
        ...
        pause_when_daily_limit=True,          # stop opening new trades when daily limit hit
        auto_flatten_on_daily_limit=True,     # flatten all positions on daily limit breach
        auto_flatten_on_session_end=False,    # flatten positions at end of session
        warning_rate_limit_seconds=60,        # rate limit identical warnings
        max_trades_per_session=20,            # safety cap
    )
...
class FixedKalshiTickerParser:
    ...
    def __init__(self):
        ...
        # Optional nicer names for logs (safe if missing)
        self.team_mappings = {
            # fill as needed, e.g. 'SEA': 'Seattle Sounders'
        }
...
class ExternalModelAdapter:
    """
    Generic adapter for external models.
    ...
    Accepted external shapes (any of these):
      ...
      - module.predict_threeway(...) / predict_outcomes(...) / get_prediction(...)
      - module.Model().predict_win_probability(...) / predict(...) / predict_threeway(...)
      - a callable module itself: impl(game_data) -> (prob, conf)|prob|dict
      - soccer 3-way: predict_match(home, away[, outcome]) or any method returning
        dict/list/np.array of {home,draw,away} or similar keys (home_win/away_win/draw)
    ...
    # --- helpers ---
    def _pick_outcome_key(self, outcome_code: str, game_data: dict) -> Optional[str]:
        ...
        if oc in ("TIE", "DRAW", "X"):
            return "draw"
        try:
            home = ((game_data.get("home_team", {}) or {}).get("abbreviation") or "").upper()
            away = ((game_data.get("away_team", {}) or {}).get("abbreviation") or "").upper()
            if oc == home:
                return "home"
            if oc == away:
                return "away"
    ...
    def _extract_prob_conf(self, out, outcome_key=None):
        # tuple/list length 2 or 1
        if isinstance(out, (tuple, list)):
            # three-way vector like [home, draw, away]
            if len(out) == 3 and outcome_key:
                idx_map = {"home": 0, "draw": 1, "away": 2, "h": 0, "x": 1, "a": 2}
                if outcome_key.lower() in idx_map:
                    prob = float(out[idx_map[outcome_key.lower()]])
                    return prob, 0.75
            if len(out) >= 2:
        ...
        try:
            if isinstance(out, np.ndarray):
                arr = out.flatten().tolist()
                return self._extract_prob_conf(arr, outcome_key)
        except Exception:
            pass
        ...
        if isinstance(out, dict):
            ...
            # outcome-style keys
            keys_lower = {k.lower(): k for k in out.keys()}
            # standard soccer keys
            for k_aliases in (("home", "h", "home_win"),
                              ("away", "a", "away_win"),
                              ("draw", "tie", "x")):
                for ka in k_aliases:
                    if ka in keys_lower:
                        # found at least one standard key; break to use below
                        break

            if outcome_key:
                candidates = [outcome_key, outcome_key.lower(), outcome_key.upper()]
                if outcome_key.lower() == "draw":
                    candidates += ["tie", "x"]
                if outcome_key.lower() == "home":
                    candidates += ["h", "home_win"]
                if outcome_key.lower() == "away":
                    candidates += ["a", "away_win"]
                for cand in candidates:
                    ...
                    if cand.lower() in keys_lower:
                        prob = float(out[keys_lower[cand.lower()]])
                        conf = float(out.get("confidence", out.get("conf", 0.75)))
                        return prob, conf
            ...
            for trio in three_sets:
                if all(k in keys_lower for k in trio):
                    if outcome_key:
                        target = keys_lower.get(outcome_key.lower())
                        if target:
                            return float(out[target]), float(out.get("confidence", 0.75))
                    # pick max if no specific outcome requested
                    best = max(trio, key=lambda k: float(out[keys_lower[k]]))
                    return float(out[keys_lower[best]]), 0.7
    ...
    def predict_win_probability(self, game_data: dict):
        ...
        # If the impl itself is callable, treat it as the predictor
        if callable(self.impl):
            raw = self._try_callable(self.impl, game_data or {}, outcome_key)
            prob, conf = self._extract_prob_conf(raw, outcome_key)
            return max(0.0, min(1.0, prob)), max(0.0, min(1.0, conf))
...
class _RateLimiter:
    """Simple per-key rate limiter for log spam control."""
...
class EnhancedModelManager:
    ...
    def __init__(self, config: InstitutionalConfig):
        ...
        self.logger = logging.getLogger(__name__)
        self.models = {}
        self.warn_limiter = _RateLimiter(getattr(config, "warning_rate_limit_seconds", 60))
    ...
    def get_prediction(self, ticker: str, game_data: Dict, sport_hint: str = None) -> Tuple[float, float, Dict]:
        ...
        try:
            ...
            # Log sparingly
            key = f"primary_model_fail:{sport}"
            if self.warn_limiter.allow(key):
                self.logger.warning(
                    f"Primary model failed for {sport} on {ticker}: {e} -> falling back"
                )
...
class RealKalshiTradingBot:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.wins = 0
        self.losses = 0
        self.suspend_trading = False  # circuit breaker state
    ...
    def _get_tradeable_price(self, ticker: str, side: str, action: str, ignore_untradeable: bool = False) -> Optional[float]:
        """
        ...
        Falls back to mid if missing. If ignore_untradeable=True, do not reject 0/1 quotes.
        """
        ...
        if price is not None and getattr(self.config, "skip_exit_on_untradeable", True) and not ignore_untradeable:
    ...
    def _execute_real_trade(self, ticker: str, decision_data: Dict) -> Optional[Position]:
        ...
        try:
            ...
            # Safety cap
            if self.total_trades >= int(getattr(self.config, "max_trades_per_session", 9999)):
                self.logger.warning("Max trades per session reached — skipping new orders.")
                return None
            ...
            self.total_trades += 1  # count on entry
    ...
    def _force_flatten_all(self):
        """Force-flatten all positions ignoring untradeable quote filter."""
        if not self.positions:
            return
        self.logger.warning("FORCE-FLATTEN: Exiting all open positions due to daily loss limit.")
        for ticker, position in list(self.positions.items()):
            # try tradeable price ignoring untradeable guard
            px = self._get_tradeable_price(ticker, position.side, action="sell", ignore_untradeable=True)
            if px is None:
                # last resort: use last mark or entry (paper-mode safety)
                px = position.last_marked_price or position.entry_price
            self._close_position(ticker, position, px, reason="circuit_breaker")
        ...
        try:
            ...
            while time.time() < end_time:
                try:
                    ...
                    if getattr(self.config, "pause_when_daily_limit", True) and self.risk_manager.daily_limit_hit():
                        if not self.suspend_trading:
                            self.suspend_trading = True
                            self.logger.warning("Daily loss limit reached — pausing new trades.")
                            if getattr(self.config, "auto_flatten_on_daily_limit", False):
                                self._force_flatten_all()
                        # When paused, just manage (should be none after flatten) and sleep
                        if self.positions:
            ...
            # session end actions
            if getattr(self.config, "auto_flatten_on_session_end", False) and self.positions:
                self._force_flatten_all()
    ...
    def _generate_session_summary(self, duration_minutes: int):
        ...
        avg_pnl_per_trade = self.total_pnl / max(1, (self.wins + self.losses))
        ...
        self.logger.info(f" Total Trades: {self.wins + self.losses}")
